...
from src.database.crud import (
    get_location,
    update_location as update_location_db,
    create_place,
    get_place as get_place_db,
    get_places,
)
...
async def update_location(
    user_id: int, payload, session: Session = Depends(get_session)
):
    try:
        ...
        await manager.broadcast(
            user_id, {"type": "location", "user_id": user_id, "data": data}
        )
...
async def ws_track(
    user_id: int, websocket: WebSocket, session: Session = Depends(get_session)
):
    try:
        ...
        if location:
            await websocket.send_json(
                {"type": "location", "user_id": user_id, "data": location}
            )
        while True:
...
async def route_to_place(
    ...
    try:
        ...
        directions = await get_directions(
            location.lat, location.lng, place.lat, place.lng, mode=mode
        )
